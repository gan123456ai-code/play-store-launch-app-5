...
const defaultAccountOptions = ['HDFC Savings', 'ICICI Salary', 'SBI Current'];




export const BankPreferencesScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const [defaultAccount, setDefaultAccount] = useState('HDFC Savings');
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(200)}>
                <GlassCard style={{ marginHorizontal: 16, marginBottom: 16, padding: 16 }} animated={false}>
                  ...
                  <View style={styles.formGroup}>
                    <Text style={[styles.formLabel, { color: theme.colors.textSecondary }]}>Default Account</Text>
                    
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
                      {defaultAccountOptions.map((opt) => (
                        <TouchableOpacity
                          key={opt}
                          onPress={() => { haptics.selection(); setDefaultAccount(opt); }}
                          style={[styles.chip, {
                            backgroundColor: defaultAccount === opt ? theme.colors.primary : 'transparent',
                            borderColor: defaultAccount === opt ? theme.colors.primary : theme.colors.border,
                          }]}
                        >
                          <Text style={[styles.chipText, { color: defaultAccount === opt ? '#FFFFFF' : theme.colors.textSecondary }]}>{opt}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  ...
                  <View style={styles.formGroup}>
                    <Text style={[styles.formLabel, { color: theme.colors.textSecondary }]}>Daily Transaction Limit</Text>
                  ...
                  <View style={[styles.toggleRow, { borderBottomColor: theme.colors.divider }]}>
                    <View>
                      <Text style={{ fontSize: 15, fontWeight: '500', color: theme.colors.text }}>Quick Pay (Skip confirmation for ₹500)</Text>
                      <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>Skip confirmation for small amounts</Text>
                    </View>
                    <Switch
                      value={quickPay}
                      onValueChange={(v) => { haptics.selection(); setQuickPay(v); }}
                  ...
                  <View style={[styles.toggleRow, { borderBottomColor: theme.colors.divider }]}>
                    <View>
                      <Text style={{ fontSize: 15, fontWeight: '500', color: theme.colors.text }}>SMS Alerts</Text>
                      <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>Receive SMS for all transactions</Text>
                    </View>
                    <Switch
                      value={smsAlerts}
                      onValueChange={(v) => { haptics.selection(); setSmsAlerts(v); }}
                      trackColor={{ true: theme.colors.success, false: theme.colors.border }}
                    />
                  </View>
