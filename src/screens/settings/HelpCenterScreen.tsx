...
const helpTopicsData = Array.from({ length: 15 }, (_, i) => ({
  id: String(i + 1),
  title: `FAQ ${i + 1}`,
  subtitle: `Common questions answered`,
  description: `Help topic ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['❓', '📖', '💡'][i % 3],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Popular', 'Booking', 'Payment'],
  features: [],
  ...
  category: ['Booking', 'Payment', 'Account', 'General'][i % 4],
  amount: 0 + (i * 500),
  imageEmoji: ['❓', '📖', '💡'][i % 3],
  details: {
...
export const HelpCenterScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'booking', 'payment', 'account', 'general'];
