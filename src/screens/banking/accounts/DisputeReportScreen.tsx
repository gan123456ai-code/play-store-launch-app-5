...
const disputeReasonsData = Array.from({ length: 8 }, (_, i) => ({
  id: String(i + 1),
  title: `Issue ${i + 1}`,
  subtitle: `Report this transaction`,
  description: `Dispute reason ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['❌', '🔄', '⚠️', '💰'][i % 4],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Fraud', 'Duplicate', 'Not Received'],
  features: [],
  ...
  category: ['Fraud', 'Service', 'Amount'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['❌', '🔄', '⚠️', '💰'][i % 4],
  details: {
...
export const DisputeReportScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'fraud', 'service', 'amount'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search report issue..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Report Issue</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('SupportChat', { itemId: item.id, item });
                    }}
