...
const statementsData = Array.from({ length: 30 }, (_, i) => ({
  id: String(i + 1),
  title: `Transaction ${i + 1}`,
  subtitle: `Account transaction entry`,
  description: `Statement entry ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['💳', '🛒', '💰', '📱'][i % 4],
  price: 100 + (i * 300),
  originalPrice: 1100 + (i * 300),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Debit', 'Credit'],
  features: ['UPI', 'Card', 'Transfer'],
...
export const AccountStatementScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'all', 'debit', 'credit'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Statement</Text>
          </View>
          {loading ? (
