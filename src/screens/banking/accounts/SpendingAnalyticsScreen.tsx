...
const spendingDataData = Array.from({ length: 8 }, (_, i) => ({
  id: String(i + 1),
  title: `Category ${i + 1}`,
  subtitle: `Monthly spending breakdown`,
  description: `Spending category ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🍔', '🛍️', '🚗', '🎬', '⚡', '💰'][i % 6],
  price: 1000 + (i * 2000),
  originalPrice: 2000 + (i * 2000),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['High', 'Medium', 'Low'],
  features: ['Up', 'Down', 'Same'],
  ...
  category: ['Food', 'Shopping', 'Transport', 'Bills'][i % 4],
  amount: 1000 + (i * 2000),
  imageEmoji: ['🍔', '🛍️', '🚗', '🎬', '⚡', '💰'][i % 6],
  details: {
...
export const SpendingAnalyticsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'food', 'shopping', 'transport', 'bills'];
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {spendingDataData.map((item, index) => (
