...
const loanTypeOptions = ['Personal', 'Home', 'Car', 'Education', 'Business'];


const employmentOptions = ['Salaried', 'Self-Employed', 'Business Owner', 'Professional'];



const section0Data = Array.from({ length: 6 }, (_, idx) => ({
  id: String(idx + 1),
  title: `Loan Plan ${idx + 1}`,
  subtitle: `Competitive interest rates`,
  description: `This is a detailed description for loan plan ${idx + 1}. It provides comprehensive information about features, pricing, availability, user ratings, and all other relevant details that premium users need to make informed decisions about their choice.`,
  icon: ['🏦', '🏠', '🚗', '📚'][idx % 4],
  ...
  tags: ['Lowest Rate', 'Quick Approval'],
  features: ['8.5% p.a.', 'Instant Approval', 'No Foreclosure'],
...
export const LoanApplicationScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const [loanAmountReq, setLoanAmountReq] = useState('');
          ...
          {loading ? (
            ...
            <>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(200)}>
                <GlassCard style={{ marginHorizontal: 16, marginBottom: 16, padding: 16 }} animated={false}>
                  ...
                  <View style={styles.formGroup}>
                    <Text style={[styles.formLabel, { color: theme.colors.textSecondary }]}>Loan Type</Text>
                    
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
                      {loanTypeOptions.map((opt) => (
                        <TouchableOpacity
                          key={opt}
                          onPress={() => { haptics.selection(); setLoanType(opt); }}
                          style={[styles.chip, {
                            backgroundColor: loanType === opt ? theme.colors.primary : 'transparent',
                            borderColor: loanType === opt ? theme.colors.primary : theme.colors.border,
                          }]}
                        >
                          <Text style={[styles.chipText, { color: loanType === opt ? '#FFFFFF' : theme.colors.textSecondary }]}>{opt}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  ...
                  <View style={styles.formGroup}>
                    <Text style={[styles.formLabel, { color: theme.colors.textSecondary }]}>Amount Required</Text>
                  ...
                  <View style={styles.formGroup}>
                    <Text style={[styles.formLabel, { color: theme.colors.textSecondary }]}>Employment</Text>
                    
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
                      {employmentOptions.map((opt) => (
                        <TouchableOpacity
                          key={opt}
                          onPress={() => { haptics.selection(); setEmployment(opt); }}
                          style={[styles.chip, {
                            backgroundColor: employment === opt ? theme.colors.primary : 'transparent',
                            borderColor: employment === opt ? theme.colors.primary : theme.colors.border,
                          }]}
                        >
                          <Text style={[styles.chipText, { color: employment === opt ? '#FFFFFF' : theme.colors.textSecondary }]}>{opt}</Text>
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  ...
                  <View style={styles.formGroup}>
                    <Text style={[styles.formLabel, { color: theme.colors.textSecondary }]}>Monthly Income</Text>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(500)} style={{ paddingHorizontal: 16, marginBottom: 16 }}>
                ...
                <GlassCard
                  onPress={() => { haptics.medium(); handleAction("Submit Application"); }}
                  style={{ marginBottom: 10, padding: 14 }}
                >
                  <View style={styles.spaceBetween}>
                    <View style={styles.row}>
                      ...
                      <View style={{ marginLeft: 12 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: theme.colors.text }}>Submit Application</Text>
