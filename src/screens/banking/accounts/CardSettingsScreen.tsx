...
const cardSettingsData = Array.from({ length: 8 }, (_, i) => ({
  id: String(i + 1),
  title: `Setting ${i + 1}`,
  subtitle: `Manage card preferences`,
  description: `Card setting ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🌏', '💳', '🔒', '📱'][i % 4],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Security', 'Transaction', 'Notification'],
  features: [],
  ...
  category: ['Security', 'Limits', 'Notifications'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['🌏', '💳', '🔒', '📱'][i % 4],
  details: {
...
export const CardSettingsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'security', 'limits', 'notifications'];
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {cardSettingsData.map((item, index) => (
