...
const txnInfoItems = [
  { id: '0', title: 'Transaction ID', subtitle: 'TXN123456789', icon: '🔢', color: '#6366F1', value: '' },
  { id: '1', title: 'Date & Time', subtitle: '20 Dec 2024, 2:30 PM', icon: '📅', color: '#10B981', value: '' },
  { id: '2', title: 'Status', subtitle: 'Completed', icon: '✅', color: '#10B981', value: '' },
  { id: '3', title: 'Category', subtitle: 'Shopping', icon: '🛍️', color: '#EC4899', value: '' },
];
...
export const TransactionDetailsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const tabs = [{ id: 'txnInfo', label: 'Transaction Info' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>📌 Transaction Info</Text>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {txnInfoItems.map((item, idx) => (
              <Animated.View key={item.id} entering={FadeInRight.duration(400).delay(idx * 60)}>
                <GlassCard
                  onPress={() => { haptics.medium(); navigation.navigate('DisputeReport'); }}
