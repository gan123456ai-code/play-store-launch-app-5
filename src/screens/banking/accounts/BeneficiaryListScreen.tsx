...
const beneficiariesData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Beneficiary ${i + 1}`,
  subtitle: `Saved bank account`,
  description: `Beneficiary ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['👤', '🏦'][i % 2],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Verified', 'Pending'],
...
export const BeneficiaryListScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Beneficiaries</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('AddBeneficiary', { itemId: item.id, item });
                    }}
