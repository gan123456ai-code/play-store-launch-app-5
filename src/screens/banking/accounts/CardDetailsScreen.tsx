...
const cardInfoItems = [
  { id: '0', title: 'Card Number', subtitle: '****8765', icon: '💳', color: '#6366F1', value: '' },
  { id: '1', title: 'Expiry Date', subtitle: '12/27', icon: '📅', color: '#F59E0B', value: '' },
  { id: '2', title: 'Status', subtitle: 'Active', icon: '✅', color: '#10B981', value: '' },
  { id: '3', title: 'Available Limit', subtitle: '₹3,87,500', icon: '💰', color: '#EC4899', value: '' },
];


export const CardDetailsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const tabs = [{ id: 'cardInfo', label: 'Card Info' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>📌 Card Info</Text>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {cardInfoItems.map((item, idx) => (
              <Animated.View key={item.id} entering={FadeInRight.duration(400).delay(idx * 60)}>
                <GlassCard
                  onPress={() => { haptics.medium(); navigation.navigate('CardSettings'); }}
