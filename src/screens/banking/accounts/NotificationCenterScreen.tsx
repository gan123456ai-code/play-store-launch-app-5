...
const notificationsData = Array.from({ length: 20 }, (_, i) => ({
  ...
  description: `Notification ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['💳', '💰', '🔒', '📱', '🎉'][i % 5],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Transaction', 'Security', 'Offer'],
  features: ['Read', 'Unread'],
  ...
  category: ['All', 'Transactions', 'Offers', 'Alerts'][i % 4],
  amount: 0 + (i * 500),
  imageEmoji: ['💳', '💰', '🔒', '📱', '🎉'][i % 5],
  details: {
...
export const NotificationCenterScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'all', 'transactions', 'offers', 'alerts'];
