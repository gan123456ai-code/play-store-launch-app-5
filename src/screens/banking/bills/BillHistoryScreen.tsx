...
const billHistoryData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Bill Payment ${i + 1}`,
  subtitle: `Past bill payment record`,
  description: `Bill ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['⚡', '💧', '📱', '🔥'][i % 4],
  price: 200 + (i * 300),
  originalPrice: 1200 + (i * 300),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Paid', 'Failed'],
  features: ['UPI', 'Card', 'Auto-Pay'],
  ...
  category: ['Electricity', 'Water', 'Mobile', 'Gas'][i % 4],
  amount: 200 + (i * 300),
  imageEmoji: ['⚡', '💧', '📱', '🔥'][i % 4],
  details: {
...
export const BillHistoryScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'electricity', 'water', 'mobile', 'gas'];
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {billHistoryData.map((item, index) => (
