...
const remindersData = Array.from({ length: 8 }, (_, i) => ({
  id: String(i + 1),
  title: `Reminder ${i + 1}`,
  subtitle: `Bill payment due`,
  description: `Reminder ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🔔', '⚡', '💧', '📱'][i % 4],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Due Soon', 'Overdue', 'Upcoming'],
  features: ['3 days left', '1 day left'],
  ...
  category: ['Urgent', 'Soon', 'Later'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['🔔', '⚡', '💧', '📱'][i % 4],
  details: {
...
export const BillRemindersScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'urgent', 'soon', 'later'];
