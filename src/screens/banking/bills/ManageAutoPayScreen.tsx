...
const autoPaysData = Array.from({ length: 5 }, (_, i) => ({
  id: String(i + 1),
  title: `Auto Pay ${i + 1}`,
  subtitle: `Recurring payment`,
  description: `Auto pay ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['⚡', '💧', '📱'][i % 3],
  price: 200 + (i * 500),
  originalPrice: 1200 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Active', 'Paused'],
  features: ['Monthly', 'Bi-Monthly'],
...
export const ManageAutoPayScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'utility', 'mobile'];
