...
const operatorsItems = [
  { id: '0', title: 'Jio', subtitle: 'Plans from ₹149', icon: '📱', color: '#0066CC', value: '' },
  { id: '1', title: 'Airtel', subtitle: 'Plans from ₹179', icon: '📱', color: '#EF4444', value: '' },
  { id: '2', title: 'Vi', subtitle: 'Plans from ₹199', icon: '📱', color: '#FFD700', value: '' },
  { id: '3', title: 'BSNL', subtitle: 'Plans from ₹107', icon: '📱', color: '#FF6B00', value: '' },
];


export const RechargeHomeScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const tabs = [{ id: 'operators', label: 'Select Operator' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>📌 Select Operator</Text>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {operatorsItems.map((item, idx) => (
              <Animated.View key={item.id} entering={FadeInRight.duration(400).delay(idx * 60)}>
                <GlassCard
                  onPress={() => { haptics.medium(); navigation.navigate('RechargePlans'); }}
