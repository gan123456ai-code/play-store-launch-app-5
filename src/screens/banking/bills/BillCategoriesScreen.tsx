...
const billCategoriesData = Array.from({ length: 12 }, (_, i) => ({
  id: String(i + 1),
  title: `Bill Category ${i + 1}`,
  subtitle: `Pay your bills quickly and securely`,
  description: `Bill category ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['⚡', '💧', '🔥', '📱', '📲', '🌐', '📡', '🛡️', '🏦', '🚗', '🎓', '🏛️'][i % 12],
  price: 0 + (i * 500),
  ...
  category: ['Utilities', 'Mobile', 'Insurance', 'Government'][i % 4],
  amount: 0 + (i * 500),
  imageEmoji: ['⚡', '💧', '🔥', '📱', '📲', '🌐', '📡', '🛡️', '🏦', '🚗', '🎓', '🏛️'][i % 12],
  details: {
...
const quickPayItems = [
  { id: '0', title: 'BSES Electricity', subtitle: 'Due: ₹2,450', icon: '⚡', color: '#F59E0B', value: '₹2,450' },
  { id: '1', title: 'Airtel Postpaid', subtitle: 'Due: ₹799', icon: '📱', color: '#EF4444', value: '₹799' },
  { id: '2', title: 'Delhi Jal Board', subtitle: 'Due: ₹650', icon: '💧', color: '#3B82F6', value: '₹650' },
];


const rechargeOptionsItems = [
  { id: '0', title: 'Mobile Prepaid', subtitle: 'Jio, Airtel, Vi, BSNL', icon: '📱', color: '#10B981', value: '' },
  { id: '1', title: 'DTH Recharge', subtitle: 'Tata Play, Dish TV, Airtel', icon: '📡', color: '#8B5CF6', value: '' },
  { id: '2', title: 'FASTag Recharge', subtitle: 'All bank FASTag', icon: '🚗', color: '#D97706', value: '' },
];


export const BillCategoriesScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'utilities', 'mobile', 'insurance', 'government'];
  const tabs = [{ id: 'quickPay', label: 'Quick Pay' }, { id: 'rechargeOptions', label: 'Recharge' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search bills & recharge..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>⚡ Quick Pay</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Previously paid bills</Text>
            </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            {quickPayItems.map((item, idx) => (
              <GlassCard
                key={item.id}
                onPress={() => { haptics.medium(); navigation.navigate('BillProviders'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(300)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>📲 Recharge</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Mobile & DTH recharge</Text>
            </View>
            <TouchableOpacity onPress={() => haptics.light()}>
              <Text style={{ fontSize: 14, fontWeight: '600', color: theme.colors.primary }}>View All</Text>
            </TouchableOpacity>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {rechargeOptionsItems.map((item, idx) => (
              <Animated.View key={item.id} entering={FadeInRight.duration(400).delay(idx * 60)}>
                <GlassCard
                  onPress={() => { haptics.medium(); navigation.navigate('RechargeHome'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(400)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Bills & Recharge</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {billCategoriesData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('BillProviders', { itemId: item.id, item });
                    }}
