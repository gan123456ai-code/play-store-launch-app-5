...
const plansData = Array.from({ length: 12 }, (_, i) => ({
  id: String(i + 1),
  title: `Plan ${i + 1}`,
  subtitle: `Unlimited calls + data + SMS`,
  description: `Recharge plan ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📱', '📡', '🌐'][i % 3],
  price: 149 + (i * 50),
  originalPrice: 1149 + (i * 50),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Popular', 'Data', 'Validity'],
  features: ['Unlimited Calls', '1.5GB/day', '100 SMS'],
  ...
  category: ['Popular', 'Data', 'Validity', 'Annual'][i % 4],
  amount: 149 + (i * 50),
  imageEmoji: ['📱', '📡', '🌐'][i % 3],
  details: {
...
export const RechargePlansScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'popular', 'data', 'validity', 'annual'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Plans</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('RechargeConfirmation', { itemId: item.id, item });
                    }}
