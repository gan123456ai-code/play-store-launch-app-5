...
const section0Data = Array.from({ length: 12 }, (_, idx) => ({
  id: String(idx + 1),
  title: `Crypto ${idx + 1}`,
  subtitle: `Cryptocurrency price`,
  description: `This is a detailed description for crypto ${idx + 1}. It provides comprehensive information about features, pricing, availability, user ratings, and all other relevant details that premium users need to make informed decisions about their choice.`,
  icon: ['₿', '🪙', '💎', '🐕'][idx % 4],
  value: `₹${(100 + idx * 5000).toLocaleString()}`,
  rating: (3.5 + (idx % 15) / 10).toFixed(1),
  reviews: 50 + idx * 30,
  status: ['available', 'limited', 'popular', 'premium'][idx % 4],
  discount: Math.floor(Math.random() * 30 + 5),
  tags: ['Bitcoin', 'Ethereum', 'Solana', 'BNB'],
  features: ['24h Change', 'Volume'],
...
export const CryptoScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
        ...
        >
          ...
          {loading ? (
            ...
            <>
                ...
                <GlassCard style={{ marginHorizontal: 16, marginBottom: 16, padding: 16 }} animated={false}>
                  ...
                  <View style={[styles.infoRow, { borderBottomColor: theme.colors.divider }]}>
                    <View style={styles.row}>
                      <View style={[styles.infoIcon, { backgroundColor: '#F7931A' + '15' }]}>
                        <Text style={{ fontSize: 16 }}>₿</Text>
                      </View>
                      <View style={{ marginLeft: 10 }}>
                        <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Bitcoin</Text>
                        <Text style={[styles.infoValue, { color: theme.colors.text }]}>0.025 BTC (₹1,25,000)</Text>
                      </View>
                    </View>
                  ...
                  <View style={[styles.infoRow, { borderBottomColor: theme.colors.divider }]}>
                    <View style={styles.row}>
                      <View style={[styles.infoIcon, { backgroundColor: '#627EEA' + '15' }]}>
                        <Text style={{ fontSize: 16 }}>🔷</Text>
                      </View>
                      <View style={{ marginLeft: 10 }}>
                        <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Ethereum</Text>
                        <Text style={[styles.infoValue, { color: theme.colors.text }]}>0.5 ETH (₹85,000)</Text>
                      </View>
                    </View>
                    <TouchableOpacity onPress={() => { haptics.light(); }}>
                      <Ionicons name="chevron-forward" size={18} color={theme.colors.textTertiary} />
              ...
              {/* Crypto Markets Items */}
              <Animated.View entering={FadeInDown.duration(400).delay(300)}>
                <View style={{ paddingHorizontal: 16, marginBottom: 8 }}>
                  <View style={styles.sectionHeader}>
                    <View>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>₿ Crypto Markets</Text>
                      <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>Live prices</Text>
                    </View>
                    <TouchableOpacity onPress={() => haptics.light()}>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(500)} style={{ paddingHorizontal: 16, marginBottom: 16 }}>
                ...
                <GlassCard
                  onPress={() => { haptics.medium(); handleAction("Buy Crypto"); }}
                  style={{ marginBottom: 10, padding: 14 }}
                >
                  <View style={styles.spaceBetween}>
                    <View style={styles.row}>
                      <View style={[styles.actionIcon, { backgroundColor: '#F7931A' + '15' }]}>
                        <Text style={{ fontSize: 20 }}>💰</Text>
                      </View>
                      <View style={{ marginLeft: 12 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: theme.colors.text }}>Buy Crypto</Text>
