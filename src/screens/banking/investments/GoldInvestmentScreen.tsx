...
const goldOptionsData = Array.from({ length: 4 }, (_, i) => ({
  id: String(i + 1),
  title: `Gold ${i + 1}`,
  subtitle: `Invest in gold`,
  description: `Gold investment ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🥇', '🏆', '💰'][i % 3],
  price: 100 + (i * 500),
  originalPrice: 1100 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Digital', 'Sovereign', 'ETF'],
  features: ['Buy from ₹1', 'No Making Charge'],
...
export const GoldInvestmentScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'digital', 'sgb', 'etf'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Gold</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {goldOptionsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('GoldBuy', { itemId: item.id, item });
                    }}
