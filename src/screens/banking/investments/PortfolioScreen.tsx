...
const portfolioItemsData = Array.from({ length: 8 }, (_, i) => ({
  id: String(i + 1),
  title: `Investment ${i + 1}`,
  subtitle: `Your portfolio holding`,
  description: `Portfolio item ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📈', '🏦', '🥇'][i % 3],
  price: 10000 + (i * 5000),
  ...
  category: ['Mutual Funds', 'FD', 'Gold'][i % 3],
  amount: 10000 + (i * 5000),
  imageEmoji: ['📈', '🏦', '🥇'][i % 3],
  details: {
...
export const PortfolioScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'mutual funds', 'fd', 'gold'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Portfolio</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('PortfolioDetails', { itemId: item.id, item });
                    }}
