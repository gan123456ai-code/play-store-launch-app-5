...
const mutualFundsData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Fund ${i + 1}`,
  subtitle: `Top performing mutual fund`,
  description: `Mutual fund ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📈', '📊', '💹', '🏆'][i % 4],
  price: 500 + (i * 100),
  originalPrice: 1500 + (i * 100),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Equity', 'Debt', 'Hybrid', 'ELSS'],
  features: ['SIP', 'Lumpsum', '5★ Rated'],
  ...
  category: ['Equity', 'Debt', 'Hybrid', 'Index', 'ELSS'][i % 5],
  amount: 500 + (i * 100),
  imageEmoji: ['📈', '📊', '💹', '🏆'][i % 4],
  details: {
...
export const MutualFundsHomeScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'equity', 'debt', 'hybrid', 'index', 'elss'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Mutual Funds</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {mutualFundsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('MutualFundDetails', { itemId: item.id, item });
                    }}
