...
const sipsData = Array.from({ length: 5 }, (_, i) => ({
  id: String(i + 1),
  title: `SIP ${i + 1}`,
  subtitle: `Monthly SIP investment`,
  description: `SIP ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📈', '🔄'][i % 2],
  price: 500 + (i * 500),
  ...
  category: ['Active', 'Completed', 'Paused'][i % 3],
  amount: 500 + (i * 500),
  imageEmoji: ['📈', '🔄'][i % 2],
  details: {
...
export const SIPManagerScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search sip manager..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All SIP Manager</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {sipsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('SIPDetails', { itemId: item.id, item });
                    }}
