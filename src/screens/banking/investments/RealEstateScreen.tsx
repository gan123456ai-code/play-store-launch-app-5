...
const section0Data = Array.from({ length: 6 }, (_, idx) => ({
  ...
  description: `This is a detailed description for property ${idx + 1}. It provides comprehensive information about features, pricing, availability, user ratings, and all other relevant details that premium users need to make informed decisions about their choice.`,
  icon: ['🏠', '🏢', '🏗️'][idx % 3],
  value: `₹${(50000 + idx * 25000).toLocaleString()}`,
  rating: (3.5 + (idx % 15) / 10).toFixed(1),
  reviews: 50 + idx * 30,
  status: ['available', 'limited', 'popular', 'premium'][idx % 4],
  discount: Math.floor(Math.random() * 30 + 5),
  tags: ['REIT', 'Fund', 'Direct'],
  features: ['8-12% p.a.', 'Monthly Rent'],
  gradient: ['#6366F1', '#8B5CF6'],
  date: `2025-01-${String(10 + idx).padStart(2, '0')}`,
  time: `${6 + idx % 12}:${idx % 2 === 0 ? '00' : '30'}`,
  amount: 50000 + idx * 25000,
...
export const RealEstateScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
        ...
        >
          ...
          {loading ? (
            ...
            <>
              ...
              {/* Real Estate Funds Items */}
              <Animated.View entering={FadeInDown.duration(400).delay(300)}>
                <View style={{ paddingHorizontal: 16, marginBottom: 8 }}>
                  <View style={styles.sectionHeader}>
                    <View>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🏠 Real Estate Funds</Text>
                      <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>Explore REITs</Text>
                    </View>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(500)} style={{ paddingHorizontal: 16, marginBottom: 16 }}>
                ...
                <GlassCard
                  onPress={() => { haptics.medium(); handleAction("Invest Now"); }}
                  style={{ marginBottom: 10, padding: 14 }}
                >
                  <View style={styles.spaceBetween}>
                    <View style={styles.row}>
                      ...
                      <View style={{ marginLeft: 12 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: theme.colors.text }}>Invest Now</Text>
