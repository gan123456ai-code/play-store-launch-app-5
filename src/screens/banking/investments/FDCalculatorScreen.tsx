...
const fdOptionsData = Array.from({ length: 5 }, (_, i) => ({
  id: String(i + 1),
  title: `FD from ${i + 1}`,
  subtitle: `Fixed deposit with guaranteed returns`,
  description: `FD ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🏦', '⭐'][i % 2],
  price: 1000 + (i * 500),
  originalPrice: 2000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Best Rate', 'Tax Saver'],
  features: ['7.5% p.a.', 'Auto-renewal'],
  ...
  category: ['Bank', 'Post Office', 'Corporate'][i % 3],
  amount: 1000 + (i * 500),
  imageEmoji: ['🏦', '⭐'][i % 2],
  details: {
...
export const FDCalculatorScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'bank', 'post office', 'corporate'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search fd calculator..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All FD Calculator</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {fdOptionsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('FDDetails', { itemId: item.id, item });
                    }}
