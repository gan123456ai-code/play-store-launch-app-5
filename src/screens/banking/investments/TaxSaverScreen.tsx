...
const taxSaverFundsData = Array.from({ length: 6 }, (_, i) => ({
  id: String(i + 1),
  title: `ELSS Fund ${i + 1}`,
  subtitle: `Tax saving mutual fund`,
  description: `ELSS ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🛡️', '📈'][i % 2],
  price: 500 + (i * 500),
  ...
  features: ['80C Benefit', '3-Year Lock-in'],
...
export const TaxSaverScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'elss', 'ppf', 'nps'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Tax Saver</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {taxSaverFundsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('TaxSaverDetails', { itemId: item.id, item });
                    }}
