...
const investmentsData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Investment ${i + 1}`,
  subtitle: `Your investment portfolio`,
  description: `Investment ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📈', '🏦', '🥇', '💎'][i % 4],
  price: 10000 + (i * 5000),
  originalPrice: 11000 + (i * 5000),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Equity', 'Debt', 'Gold', 'FD'],
  features: ['High Returns', 'Tax Benefit', 'SIP'],
  ...
  category: ['Mutual Funds', 'FD', 'Gold', 'SIP'][i % 4],
  amount: 10000 + (i * 5000),
  imageEmoji: ['📈', '🏦', '🥇', '💎'][i % 4],
  details: {
...
const portfolioOverviewItems = [
  { id: '0', title: 'Total Investment', subtitle: '₹5,86,000', icon: '💰', color: '#6366F1', value: '₹5,86,000' },
  { id: '1', title: 'Current Value', subtitle: '₹7,12,000', icon: '📈', color: '#10B981', value: '₹7,12,000' },
  { id: '2', title: 'Returns', subtitle: '+₹1,26,000 (21.5%)', icon: '🎉', color: '#F59E0B', value: '+21.5%' },
];


const investOptionsItems = [
  { id: '0', title: 'Mutual Funds', subtitle: 'SIP & Lumpsum investments', icon: '📈', color: '#6366F1', value: '' },
  { id: '1', title: 'Fixed Deposits', subtitle: 'Guaranteed returns up to 7.5%', icon: '🏦', color: '#10B981', value: '' },
  { id: '2', title: 'Digital Gold', subtitle: 'Buy gold starting from ₹1', icon: '🥇', color: '#F59E0B', value: '' },
  { id: '3', title: 'Tax Saving (ELSS)', subtitle: 'Save tax under Section 80C', icon: '🛡️', color: '#EC4899', value: '' },
];


export const InvestmentsHomeScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'mutual funds', 'fd', 'gold', 'sip'];
  const tabs = [{ id: 'portfolioOverview', label: 'Portfolio Overview' }, { id: 'investOptions', label: 'Invest In' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>💼 Portfolio Overview</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Your investment summary</Text>
            </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            {portfolioOverviewItems.map((item, idx) => (
              <GlassCard
                key={item.id}
                onPress={() => { haptics.medium(); navigation.navigate('MutualFundsHome'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(300)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🚀 Invest In</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Explore investment options</Text>
            </View>
            <TouchableOpacity onPress={() => haptics.light()}>
              <Text style={{ fontSize: 14, fontWeight: '600', color: theme.colors.primary }}>View All</Text>
            </TouchableOpacity>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {investOptionsItems.map((item, idx) => (
              <Animated.View key={item.id} entering={FadeInRight.duration(400).delay(idx * 60)}>
                <GlassCard
                  onPress={() => { haptics.medium(); navigation.navigate('FDCalculator'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(400)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Investments</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('MutualFundsHome', { itemId: item.id, item });
                    }}
