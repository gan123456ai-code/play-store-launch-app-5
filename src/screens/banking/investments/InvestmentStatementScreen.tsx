...
const investmentEntriesData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Entry ${i + 1}`,
  subtitle: `Investment transaction`,
  description: `Entry ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['💰', '📈'][i % 2],
  price: 500 + (i * 500),
  originalPrice: 1500 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Buy', 'Sell', 'Dividend'],
  features: [],
...
export const InvestmentStatementScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'all', 'buy', 'sell'];
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {investmentEntriesData.map((item, index) => (
