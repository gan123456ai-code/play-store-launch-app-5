...
const section0Data = Array.from({ length: 8 }, (_, idx) => ({
  id: String(idx + 1),
  title: `Bond ${idx + 1}`,
  subtitle: `Fixed income bond investment`,
  description: `This is a detailed description for bond ${idx + 1}. It provides comprehensive information about features, pricing, availability, user ratings, and all other relevant details that premium users need to make informed decisions about their choice.`,
  icon: ['📄', '🏦', '🏛️'][idx % 3],
  value: `₹${(10000 + idx * 5000).toLocaleString()}`,
  rating: (3.5 + (idx % 15) / 10).toFixed(1),
  reviews: 50 + idx * 30,
  status: ['available', 'limited', 'popular', 'premium'][idx % 4],
  discount: Math.floor(Math.random() * 30 + 5),
  tags: ['Government', 'Corporate', 'Tax-Free'],
  features: ['7.5% p.a.', 'AAA Rated'],
  gradient: ['#6366F1', '#8B5CF6'],
  date: `2025-01-${String(10 + idx).padStart(2, '0')}`,
  time: `${6 + idx % 12}:${idx % 2 === 0 ? '00' : '30'}`,
  amount: 10000 + idx * 5000,
...
export const BondMarketScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
          ...
          {loading ? (
            ...
            <>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(500)} style={{ paddingHorizontal: 16, marginBottom: 16 }}>
                ...
                <GlassCard
                  onPress={() => { haptics.medium(); handleAction("Invest in Bonds"); }}
                  style={{ marginBottom: 10, padding: 14 }}
                >
                  <View style={styles.spaceBetween}>
                    <View style={styles.row}>
                      ...
                      <View style={{ marginLeft: 12 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: theme.colors.text }}>Invest in Bonds</Text>
