...
const pendingRequestsData = Array.from({ length: 5 }, (_, i) => ({
  id: String(i + 1),
  title: `Request from ${i + 1}`,
  subtitle: `Pending payment request`,
  description: `Request ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📩', '💰'][i % 2],
  price: 500 + (i * 500),
  originalPrice: 1500 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Pending', 'Completed'],
  features: ['UPI', 'Reminder Sent'],
...
export const RequestMoneyScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
