...
const allContactsData = Array.from({ length: 12 }, (_, i) => ({
  id: String(i + 1),
  title: `Contact ${i + 1}`,
  subtitle: `Verified payee`,
  description: `Contact ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['👤', '👩', '👨'][i % 3],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Favorite', 'Verified'],
  features: ['UPI', 'Bank Account'],
  ...
  category: ['Favorites', 'Recent', 'All'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['👤', '👩', '👨'][i % 3],
  details: {
...
export const ContactsListScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'favorites', 'recent', 'all'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Select Contact</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('EnterAmount', { itemId: item.id, item });
                    }}
