...
const card0Data = {
  ...
  change: '+8.5%',
  changePositive: true,
  subtitle: 'Including all accounts and investments',
  icon: '💰',
  gradient: ['#059669', '#10B981'],
  stats: [
    { label: 'Savings', value: '₹18,55,440', icon: '🏦' },
    { label: 'Investments', value: '₹7,12,000', icon: '📈' },
    { label: 'FD', value: '₹2,88,560', icon: '🏛️' },
  ],
};
...
const card1Data = {
  ...
  change: '+5%',
  changePositive: true,
  subtitle: 'Salary + Interest + Dividends',
  icon: '💵',
  gradient: ['#6366F1', '#8B5CF6'],
  stats: [
    { label: 'Salary', value: '₹1,05,000', icon: '💼' },
    { label: 'Interest', value: '₹12,000', icon: '🏦' },
    { label: 'Dividends', value: '₹8,000', icon: '📈' },
  ],
};
...
const card2Data = {
  ...
  subtitle: 'Lower than last month average',
  icon: '📉',
...
const chart0Data = Array.from({ length: 12 }, (_, idx) => ({
  ...
  value: 50000 + Math.floor(Math.random() * 80000),
  previousValue: 50000 + Math.floor(Math.random() * 80000),
  color: ['#10B981', '#EF4444', '#6366F1', '#F59E0B'][idx % 4],
  percentage: Math.floor(Math.random() * 100),
}));


const chart1Data = Array.from({ length: 8 }, (_, idx) => ({
  label: ['Food', 'Rent', 'Transport', 'Shopping', 'Bills', 'Health', 'Travel', 'Other'][idx % 12],
  value: 2000 + Math.floor(Math.random() * 15000),
  previousValue: 2000 + Math.floor(Math.random() * 15000),
...
const list0Data = Array.from({ length: 8 }, (_, idx) => ({
  ...
  description: `This is a comprehensive description for category ${idx + 1}. It contains all relevant details about pricing, features, availability, reviews, and everything else that premium users need to make informed decisions. Our platform provides the best experience with top-notch quality and reliability. Each item is carefully curated for maximum value and satisfaction.`,
  icon: ['🍔', '🏠', '🚗', '🛍️', '⚡', '🏥'][idx % 6],
  value: '₹' + (5000 + idx * 3000).toLocaleString(),
  amount: 5000 + idx * 3000,
...
export const BankingAnalyticsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
        ...
        {loading ? (
          ...
          <>
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(240)}>
              <TouchableOpacity
                onPress={() => { haptics.medium(); setExpandedCard(expandedCard === 3 ? null : 3); }}
                activeOpacity={0.9}
                style={{ marginHorizontal: 16, marginBottom: 12 }}
              >
                <LinearGradient
                  colors={card3Data.gradient}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
                  style={styles.summaryCard}
                >
                  <View style={styles.cardHeader}>
                    <View>
                      <Text style={styles.cardIcon}>{card3Data.icon}</Text>
                      <Text style={styles.cardLabel}>{card3Data.title}</Text>
                      <Text style={styles.cardValue}>{card3Data.value}</Text>
                    </View>
                    <View style={styles.changeContainer}>
                      <Ionicons name={card3Data.changePositive ? 'trending-up' : 'trending-down'} size={18} color={card3Data.changePositive ? '#4ADE80' : '#FB7185'} />
                      <Text style={[styles.changeText, { color: card3Data.changePositive ? '#4ADE80' : '#FB7185' }]}>{card3Data.change}</Text>
                    </View>
                  </View>
                  <Text style={styles.cardSubtitle}>{card3Data.subtitle}</Text>

                  {/* Expanded Stats */}
                  {expandedCard === 3 && card3Data.stats.length > 0 && (
                    <Animated.View entering={FadeInDown.duration(300)} style={styles.statsRow}>
                      {card3Data.stats.map((stat, sIdx) => (
                        <View key={sIdx} style={styles.statItem}>
                          <Text style={styles.statIcon}>{stat.icon}</Text>
                          <Text style={styles.statValue}>{stat.value}</Text>
                          <Text style={styles.statLabel}>{stat.label}</Text>
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(320)}>
              <GlassCard style={{ marginHorizontal: 16, marginBottom: 16, padding: 16 }} animated={false}>
                <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.text, marginBottom: 12 }}>Income vs Expense</Text>
                {/* Visual Chart Bars */}
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(400)}>
              <GlassCard style={{ marginHorizontal: 16, marginBottom: 16, padding: 16 }} animated={false}>
                <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.text, marginBottom: 12 }}>Spending Breakdown</Text>
                {/* Visual Chart Bars */}
                <View style={styles.chartContainer}>
              ...
              <View style={{ paddingHorizontal: 16, marginBottom: 8 }}>
                <View style={styles.sectionHeader}>
                  <View>
                    ...
                    <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Where your money goes</Text>
                  </View>
            ...
            {/* Quick Actions */}
            <Animated.View entering={FadeInDown.duration(400).delay(560)} style={{ paddingHorizontal: 16, marginBottom: 16 }}>
              ...
              <View style={styles.actionsGrid}>
                ...
                >
                  ...
                  <Text style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center' }}>Monthly</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={[styles.actionLabel, { color: theme.colors.text }]}>Budget</Text>
                  <Text style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center' }}>Set limits</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center' }}>Statement</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={[styles.actionLabel, { color: theme.colors.text }]}>Tips</Text>
                  <Text style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center' }}>Save more</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center' }}>Overspend</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text style={{ fontSize: 10, color: theme.colors.textTertiary, textAlign: 'center' }}>Payments</Text>
                </TouchableOpacity>
