...
const hotelResultsData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Hotel ${i + 1}`,
  subtitle: `Luxury rooms with premium amenities and city view`,
  description: `Premium hotel with excellent reviews ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🏨', '🏰', '🌴', '⭐', '🌃'][i % 5],
  price: 3500 + (i * 1200),
  originalPrice: 4500 + (i * 1200),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['5-Star', '4-Star', '3-Star', 'Boutique'],
  ...
  category: ['5-Star', '4-Star', '3-Star', 'Budget'][i % 4],
  amount: 3500 + (i * 1200),
  imageEmoji: ['🏨', '🏰', '🌴', '⭐', '🌃'][i % 5],
  details: {
...
export const HotelResultsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', '5-star', '4-star', '3-star', 'budget'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search hotel results..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Hotel Results</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {hotelResultsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('HotelDetails', { itemId: item.id, item });
                    }}
