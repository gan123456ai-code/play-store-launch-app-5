...
const compareHotelsData = Array.from({ length: 4 }, (_, i) => ({
  id: String(i + 1),
  title: `Hotel ${i + 1}`,
  subtitle: `Compare features and prices`,
  description: `Hotel comparison ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🏨', '🏰', '🌴', '⭐'][i % 4],
  price: 3500 + (i * 2000),
  originalPrice: 4500 + (i * 2000),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Best Value', 'Premium', 'Budget'],
  ...
  category: ['5-Star', '4-Star', '3-Star'][i % 3],
  amount: 3500 + (i * 2000),
  imageEmoji: ['🏨', '🏰', '🌴', '⭐'][i % 4],
  details: {
...
export const HotelCompareScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {compareHotelsData.map((item, index) => (
