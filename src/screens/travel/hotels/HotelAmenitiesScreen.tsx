...
const amenityListData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Amenity ${i + 1}`,
  subtitle: `Premium facility available at the hotel`,
  description: `Hotel amenity ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📶', '🏊', '💆', '💪', '🍽️', '🅿️'][i % 6],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Free', 'Paid', 'Premium'],
  features: ['Available', '24/7', 'On Request'],
  ...
  category: ['General', 'Wellness', 'Dining', 'Business'][i % 4],
  amount: 0 + (i * 500),
  imageEmoji: ['📶', '🏊', '💆', '💪', '🍽️', '🅿️'][i % 6],
  details: {
...
export const HotelAmenitiesScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'general', 'wellness', 'dining', 'business'];
      ...
      >
        ...
        {loading ? (
          ...
          <View style={{ paddingHorizontal: 16 }}>
            ...
            {amenityListData.map((item, index) => (
