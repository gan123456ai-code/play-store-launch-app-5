...
const availableRoomsData = Array.from({ length: 6 }, (_, i) => ({
  id: String(i + 1),
  title: `Room ${i + 1}`,
  subtitle: `Comfortable room with all modern amenities`,
  description: `Premium room ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🛏️', '🛋️', '👑'][i % 3],
  price: 3500 + (i * 2500),
  originalPrice: 4500 + (i * 2500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Standard', 'Deluxe', 'Suite'],
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Select Room</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('HotelBookingForm', { itemId: item.id, item });
                    }}
