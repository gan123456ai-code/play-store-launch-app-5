...
const section0Data = Array.from({ length: 15 }, (_, idx) => ({
  id: String(idx + 1),
  title: `Menu Item ${idx + 1}`,
  subtitle: `Delicious meal delivered to your room`,
  description: `This is a detailed description for menu item ${idx + 1}. It provides comprehensive information about features, pricing, availability, user ratings, and all other relevant details that premium users need to make informed decisions about their choice.`,
  icon: ['🍛', '🍝', '🍕', '🥗', '🍰', '☕', '🍷'][idx % 7],
  value: `₹${(250 + idx * 150).toLocaleString()}`,
  rating: (3.5 + (idx % 15) / 10).toFixed(1),
  reviews: 50 + idx * 30,
  status: ['available', 'limited', 'popular', 'premium'][idx % 4],
  discount: Math.floor(Math.random() * 30 + 5),
  tags: ['Veg', 'Non-Veg', 'Dessert', 'Beverage'],
  features: ['30 min delivery', 'Hot & Fresh'],
...
export const HotelRoomServiceScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
          ...
          {loading ? (
            ...
            <>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(300)}>
                <View style={{ paddingHorizontal: 16, marginBottom: 8 }}>
                  <View style={styles.sectionHeader}>
                    <View>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🍽️ Room Service Menu</Text>
                      <Text style={{ fontSize: 12, color: theme.colors.textSecondary }}>Order now</Text>
                    </View>
              ...
              <Animated.View entering={FadeInDown.duration(400).delay(500)} style={{ paddingHorizontal: 16, marginBottom: 16 }}>
                ...
                <GlassCard
                  onPress={() => { haptics.medium(); handleAction("Place Order"); }}
                  style={{ marginBottom: 10, padding: 14 }}
                >
                  <View style={styles.spaceBetween}>
                    <View style={styles.row}>
                      ...
                      <View style={{ marginLeft: 12 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: theme.colors.text }}>Place Order</Text>
