...
const hotelsData = Array.from({ length: 15 }, (_, i) => ({
  id: String(i + 1),
  title: `Premium Hotel ${i + 1}`,
  subtitle: `Luxury accommodation with world-class amenities`,
  description: `Experience the finest hospitality at hotel ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🏨', '🏰', '🌴', '⭐', '🏖️'][i % 5],
  price: 3500 + (i * 800),
  originalPrice: 4500 + (i * 800),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Luxury', 'Premium', 'Budget', 'Business'],
  features: ['Free WiFi', 'Pool', 'Spa', 'Gym', 'Restaurant'],
  ...
  category: ['Luxury', 'Business', 'Budget', 'Resort'][i % 4],
  amount: 3500 + (i * 800),
  imageEmoji: ['🏨', '🏰', '🌴', '⭐', '🏖️'][i % 5],
  details: {
...
const popularDestItems = [
  { id: '0', title: 'Goa', subtitle: '150+ hotels from ₹1,999', icon: '🏖️', color: '#F97316', value: 'From ₹1,999' },
  { id: '1', title: 'Jaipur', subtitle: '120+ hotels from ₹1,499', icon: '🏰', color: '#8B5CF6', value: 'From ₹1,499' },
  { id: '2', title: 'Mumbai', subtitle: '300+ hotels from ₹2,499', icon: '🌃', color: '#06B6D4', value: 'From ₹2,499' },
  { id: '3', title: 'Udaipur', subtitle: '80+ hotels from ₹2,999', icon: '🏛️', color: '#EC4899', value: 'From ₹2,999' },
  { id: '4', title: 'Dubai', subtitle: '200+ hotels from ₹8,999', icon: '🏙️', color: '#D97706', value: 'From ₹8,999' },
];


const hotelDealsItems = [
  { id: '0', title: 'Flat 40% off on 5-star hotels', subtitle: 'Valid till 31st January', icon: '⭐', color: '#F59E0B', value: 'Code: STAR40' },
  { id: '1', title: 'Free breakfast on all bookings', subtitle: 'Minimum 2 nights stay', icon: '🥐', color: '#10B981', value: 'Auto-applied' },
  { id: '2', title: 'Pay at hotel - No advance', subtitle: 'Available at select properties', icon: '🏦', color: '#6366F1', value: 'Book Now' },
];


export const HotelSearchScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'luxury', 'business', 'budget', 'resort'];
  const tabs = [{ id: 'popularDest', label: 'Popular Destinations' }, { id: 'hotelDeals', label: 'Best Deals' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🌍 Popular Destinations</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Trending places to stay</Text>
            </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            {popularDestItems.map((item, idx) => (
              <GlassCard
                key={item.id}
                onPress={() => { haptics.medium(); navigation.navigate('HotelResults'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(300)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 Best Deals</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Save big on top hotels</Text>
            </View>
            <TouchableOpacity onPress={() => haptics.light()}>
              <Text style={{ fontSize: 14, fontWeight: '600', color: theme.colors.primary }}>View All</Text>
            </TouchableOpacity>
          ...
          <View style={{ paddingHorizontal: 16 }}>
            {hotelDealsItems.map((item, idx) => (
              <Animated.View key={item.id} entering={FadeInRight.duration(400).delay(idx * 60)}>
                <GlassCard
                  onPress={() => { haptics.medium(); navigation.navigate('HotelDetails'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(400)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Hotels</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('HotelResults', { itemId: item.id, item });
                    }}
