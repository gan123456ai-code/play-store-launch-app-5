...
const photosData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Photo ${i + 1}`,
  subtitle: `Hotel interior and exterior photos`,
  description: `Photo ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🏨', '🛏️', '🏊', '🍽️', '🌅'][i % 5],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Room', 'Pool', 'Restaurant', 'Lobby'],
  features: [],
  ...
  category: ['Interior', 'Exterior', 'Room', 'Amenities'][i % 4],
  amount: 0 + (i * 500),
  imageEmoji: ['🏨', '🛏️', '🏊', '🍽️', '🌅'][i % 5],
  details: {
...
export const HotelGalleryScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'interior', 'exterior', 'room', 'amenities'];
