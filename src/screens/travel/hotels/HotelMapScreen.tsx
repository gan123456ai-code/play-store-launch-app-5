...
const mapHotelsData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Hotel ${i + 1}`,
  subtitle: `View on map`,
  description: `Hotel location ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📍', '🏨', '🗺️'][i % 3],
  price: 3500 + (i * 1500),
  originalPrice: 4500 + (i * 1500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Nearby', 'Central'],
  features: ['City Center', 'Near Airport', 'Beach Side'],
  ...
  category: ['Central', 'Suburban', 'Beach'][i % 3],
  amount: 3500 + (i * 1500),
  imageEmoji: ['📍', '🏨', '🗺️'][i % 3],
  details: {
...
export const HotelMapScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'central', 'suburban', 'beach'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Map View</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {mapHotelsData.map((item, index) => (
