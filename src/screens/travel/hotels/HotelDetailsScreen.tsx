...
const roomsData = Array.from({ length: 6 }, (_, i) => ({
  id: String(i + 1),
  title: `Room Type ${i + 1}`,
  subtitle: `Spacious room with modern amenities`,
  description: `Luxurious room ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🛏️', '🛋️', '👑', '🌟'][i % 4],
  price: 3500 + (i * 2000),
  originalPrice: 4500 + (i * 2000),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Standard', 'Deluxe', 'Suite', 'Presidential'],
  features: ['King Bed', 'City View', 'Minibar', 'Bathtub'],
  ...
  category: ['Standard', 'Deluxe', 'Suite'][i % 3],
  amount: 3500 + (i * 2000),
  imageEmoji: ['🛏️', '🛋️', '👑', '🌟'][i % 4],
  details: {
...
const amenitiesItems = [
  { id: '0', title: 'Free WiFi', subtitle: 'High-speed internet', icon: '📶', color: '#3B82F6', value: '' },
  { id: '1', title: 'Swimming Pool', subtitle: 'Outdoor heated pool', icon: '🏊', color: '#06B6D4', value: '' },
  { id: '2', title: 'Spa & Massage', subtitle: 'Full service spa', icon: '💆', color: '#EC4899', value: '' },
  { id: '3', title: 'Fitness Center', subtitle: '24/7 access', icon: '💪', color: '#10B981', value: '' },
  { id: '4', title: 'Restaurant', subtitle: 'Multi-cuisine dining', icon: '🍽️', color: '#F59E0B', value: '' },
];
...
export const HotelDetailsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const tabs = [{ id: 'amenities', label: 'Amenities' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search hotel details..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>📌 Amenities</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            {amenitiesItems.map((item, idx) => (
              <GlassCard
                key={item.id}
                onPress={() => { haptics.medium(); navigation.navigate('RoomSelection'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(300)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Hotel Details</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('RoomSelection', { itemId: item.id, item });
                    }}
