...
const policiesData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Policy ${i + 1}`,
  subtitle: `Important hotel policy information`,
  description: `Policy ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🕐', '❌', '👶', '🐾', '🚭'][i % 5],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Important', 'Standard', 'Special'],
  features: [],
  ...
  category: ['General', 'Cancellation', 'Safety'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['🕐', '❌', '👶', '🐾', '🚭'][i % 5],
  details: {
...
export const HotelPoliciesScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'general', 'cancellation', 'safety'];
