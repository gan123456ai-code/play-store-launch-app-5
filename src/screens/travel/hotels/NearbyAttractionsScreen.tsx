...
const nearbyData = Array.from({ length: 15 }, (_, i) => ({
  id: String(i + 1),
  title: `Attraction ${i + 1}`,
  subtitle: `Popular place near the hotel`,
  description: `Nearby attraction ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🏛️', '🛍️', '🌳', '🍜', '🚇', '🎭'][i % 6],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Popular', 'Must Visit', 'Hidden Gem'],
  features: ['Walking Distance', 'Taxi Required', 'Metro Connected'],
  ...
  category: ['Shopping', 'Culture', 'Nature', 'Food'][i % 4],
  amount: 0 + (i * 500),
  imageEmoji: ['🏛️', '🛍️', '🌳', '🍜', '🚇', '🎭'][i % 6],
  details: {
...
export const NearbyAttractionsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'shopping', 'culture', 'nature', 'food'];
