...
const card0Data = {
  ...
  change: '+2 this year',
  changePositive: true,
  subtitle: 'India, Thailand, Singapore, UAE, UK',
  icon: '🌍',
...
const chart0Data = Array.from({ length: 6 }, (_, idx) => ({
  ...
  previousValue: 1 + Math.floor(Math.random() * 4),
  color: ['#6366F1'][idx % 1],
...
const list0Data = Array.from({ length: 12 }, (_, idx) => ({
  id: String(idx + 1),
  title: `City ${idx + 1}`,
  subtitle: `Explored city`,
  description: `This is a comprehensive description for city ${idx + 1}. It contains all relevant details about pricing, features, availability, reviews, and everything else that premium users need to make informed decisions. Our platform provides the best experience with top-notch quality and reliability. Each item is carefully curated for maximum value and satisfaction.`,
  icon: ['🗼', '🏖️', '🏔️', '🌴', '🏰'][idx % 5],
  value: '₹' + (1000 + idx * 500).toLocaleString(),
  amount: 1000 + idx * 500,
...
export const TravelMapScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  return (
    <GradientBackground>
      <PremiumHeader
        title="Travel Map"
        subtitle="🗺️ Dashboard & Analytics"
          ...
          <>
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(160)}>
              <GlassCard style={{ marginHorizontal: 16, marginBottom: 16, padding: 16 }} animated={false}>
                <Text style={{ fontSize: 16, fontWeight: '700', color: theme.colors.text, marginBottom: 12 }}>Travel Timeline</Text>
                {/* Visual Chart Bars */}
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(240)}>
              <View style={{ paddingHorizontal: 16, marginBottom: 8 }}>
                <View style={styles.sectionHeader}>
                  <View>
                    <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🏙️ Cities Visited</Text>
                    <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Your travel footprint</Text>
                  </View>
