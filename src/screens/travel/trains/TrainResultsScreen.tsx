...
const trainsData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Train ${i + 1}`,
  subtitle: `Express train with pantry and charging`,
  description: `Train ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🚂', '🚄', '🚃'][i % 3],
  price: 350 + (i * 150),
  originalPrice: 1350 + (i * 150),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Rajdhani', 'Shatabdi', 'Superfast'],
  features: ['AC 3-Tier', 'Sleeper', 'Chair Car'],
  ...
  category: ['AC', 'Sleeper', 'Chair Car'][i % 3],
  amount: 350 + (i * 150),
  imageEmoji: ['🚂', '🚄', '🚃'][i % 3],
  details: {
...
export const TrainResultsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'ac', 'sleeper', 'chair car'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search train results..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Train Results</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {trainsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('TrainSeatSelection', { itemId: item.id, item });
                    }}
