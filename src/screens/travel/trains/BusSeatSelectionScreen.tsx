...
const busSeatsData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Seat ${i + 1}`,
  subtitle: `Available seat`,
  description: `Bus seat ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['💺', '🛏️'][i % 2],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Window', 'Aisle', 'Lower'],
  features: ['Lower Deck', 'Upper Deck'],
  ...
  category: ['Window', 'Aisle', 'Middle'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['💺', '🛏️'][i % 2],
  details: {
...
export const BusSeatSelectionScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search select seat..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Select Seat</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {busSeatsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('BusBookingForm', { itemId: item.id, item });
                    }}
