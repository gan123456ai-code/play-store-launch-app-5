...
const busRoutesData = Array.from({ length: 12 }, (_, i) => ({
  id: String(i + 1),
  title: `Bus Route ${i + 1}`,
  subtitle: `Comfortable overnight journey`,
  description: `Bus route ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🚌', '🚍'][i % 2],
  price: 400 + (i * 200),
  originalPrice: 1400 + (i * 200),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['AC Sleeper', 'Volvo', 'Non-AC'],
  features: ['WiFi', 'Charging', 'Blanket'],
  ...
  category: ['Sleeper', 'Semi-Sleeper', 'Seater'][i % 3],
  amount: 400 + (i * 200),
  imageEmoji: ['🚌', '🚍'][i % 2],
  details: {
...
export const BusSearchScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'sleeper', 'semi-sleeper', 'seater'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Bus Search</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('BusResults', { itemId: item.id, item });
                    }}
