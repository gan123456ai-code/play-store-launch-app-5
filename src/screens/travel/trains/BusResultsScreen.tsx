...
const busesData = Array.from({ length: 15 }, (_, i) => ({
  id: String(i + 1),
  title: `Bus ${i + 1}`,
  subtitle: `Overnight AC sleeper bus`,
  description: `Bus ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🚌', '🚍', '🚎'][i % 3],
  price: 500 + (i * 150),
  originalPrice: 1500 + (i * 150),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['AC', 'Non-AC', 'Volvo'],
  features: ['Sleeper', 'Charging', 'WiFi'],
  ...
  category: ['AC Sleeper', 'Semi-Sleeper', 'Seater'][i % 3],
  amount: 500 + (i * 150),
  imageEmoji: ['🚌', '🚍', '🚎'][i % 3],
  details: {
...
export const BusResultsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'ac sleeper', 'semi-sleeper', 'seater'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search bus results..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Bus Results</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('BusDetails', { itemId: item.id, item });
                    }}
