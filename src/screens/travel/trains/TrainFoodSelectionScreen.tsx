...
const foodItemsData = Array.from({ length: 15 }, (_, i) => ({
  id: String(i + 1),
  title: `Food Item ${i + 1}`,
  subtitle: `Delicious meal prepared fresh`,
  description: `Train meal ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🍛', '🍱', '🥪', '☕', '🍨'][i % 5],
  price: 50 + (i * 30),
  originalPrice: 1050 + (i * 30),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Veg', 'Non-Veg', 'Snacks', 'Beverages'],
  features: ['Fresh', 'Hot', 'Packaged'],
  ...
  category: ['Meals', 'Snacks', 'Beverages', 'Desserts'][i % 4],
  amount: 50 + (i * 30),
  imageEmoji: ['🍛', '🍱', '🥪', '☕', '🍨'][i % 5],
  details: {
...
export const TrainFoodSelectionScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'meals', 'snacks', 'beverages', 'desserts'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Food & Beverages</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('TrainPayment', { itemId: item.id, item });
                    }}
