...
const platformsData = Array.from({ length: 8 }, (_, i) => ({
  id: String(i + 1),
  title: `Platform ${i + 1}`,
  subtitle: `Station facilities`,
  description: `Platform ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🚉', '🪧'][i % 2],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Active', 'Under Maintenance'],
  features: ['Elevator', 'Escalator', 'Waiting Room'],
...
export const PlatformInfoScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'main', 'suburban'];
