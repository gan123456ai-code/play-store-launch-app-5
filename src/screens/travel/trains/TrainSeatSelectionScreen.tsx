...
const berthsData = Array.from({ length: 10 }, (_, i) => ({
  id: String(i + 1),
  title: `Berth ${i + 1}`,
  subtitle: `Available berth`,
  description: `Berth ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🛏️', '💺'][i % 2],
  price: 0 + (i * 500),
  originalPrice: 1000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Lower', 'Middle', 'Upper', 'Side'],
  features: ['Lower Berth', 'Window Side'],
  ...
  category: ['Lower', 'Middle', 'Upper'][i % 3],
  amount: 0 + (i * 500),
  imageEmoji: ['🛏️', '💺'][i % 2],
  details: {
...
export const TrainSeatSelectionScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'lower', 'middle', 'upper'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search select berth..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Select Berth</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {berthsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('TrainPassengerDetails', { itemId: item.id, item });
                    }}
