...
const section0Items = Array.from({ length: 12 }, (_, idx) => ({
  id: `item_${idx}`,
  title: `Flight Deal ${idx + 1}`,
  subtitle: `Exclusive airfare offer`,
  description: `Detailed description for flight deal ${idx + 1}. This premium item offers exceptional quality and value. Features include top-rated service, verified reviews from thousands of users, competitive pricing with exclusive member discounts, and 24/7 customer support. Available across multiple locations with easy booking and instant confirmation. Rated ${(4 + Math.random()).toFixed(1)} stars by over ${100 + idx * 50} verified users.`,
  icon: ['✈️', '🛫', '🌍'][idx % 3],
  amount: 1999 + idx * 800,
  rating: Number((3.8 + (idx % 12) / 10).toFixed(1)),
  reviews: 50 + idx * 25,
  status: ['active', 'pending', 'completed', 'featured'][idx % 4],
  tags: ['Domestic', 'International', 'Weekend'].slice(0, 2 + idx % 2),
  ...
  trending: idx % 4 === 0,
  details: {
    line1: `Key detail: Premium quality flight deal with verified certifications`,
...
const section1Items = Array.from({ length: 10 }, (_, idx) => ({
  id: `item_${idx}`,
  title: `Hotel Deal ${idx + 1}`,
  subtitle: `Premium stay offer`,
  description: `Detailed description for hotel deal ${idx + 1}. This premium item offers exceptional quality and value. Features include top-rated service, verified reviews from thousands of users, competitive pricing with exclusive member discounts, and 24/7 customer support. Available across multiple locations with easy booking and instant confirmation. Rated ${(4 + Math.random()).toFixed(1)} stars by over ${100 + idx * 50} verified users.`,
  icon: ['🏨', '🏖️', '🏔️'][idx % 3],
  amount: 2499 + idx * 1000,
  rating: Number((3.8 + (idx % 12) / 10).toFixed(1)),
  reviews: 50 + idx * 25,
  status: ['active', 'pending', 'completed', 'featured'][idx % 4],
  tags: ['Luxury', 'Budget', 'Resort'].slice(0, 2 + idx % 2),
  ...
  trending: idx % 4 === 0,
  details: {
    line1: `Key detail: Premium quality hotel deal with verified certifications`,
    line2: `Additional info: Available 24/7 with instant confirmation and support`,
    line3: `Note: Member exclusive pricing with loyalty points on every transaction`,
    stat1: { label: 'Rating', value: `${(4 + Math.random()).toFixed(1)}★` },
    stat2: { label: 'Users', value: `${(100 + idx * 50).toLocaleString()}` },
...
const section2Items = Array.from({ length: 8 }, (_, idx) => ({
  id: `item_${idx}`,
  title: `Travel Package ${idx + 1}`,
  subtitle: `All-inclusive package`,
  description: `Detailed description for travel package ${idx + 1}. This premium item offers exceptional quality and value. Features include top-rated service, verified reviews from thousands of users, competitive pricing with exclusive member discounts, and 24/7 customer support. Available across multiple locations with easy booking and instant confirmation. Rated ${(4 + Math.random()).toFixed(1)} stars by over ${100 + idx * 50} verified users.`,
  icon: ['🗺️', '🏝️', '🗼'][idx % 3],
  amount: 9999 + idx * 5000,
  rating: Number((3.8 + (idx % 12) / 10).toFixed(1)),
  reviews: 50 + idx * 25,
  status: ['active', 'pending', 'completed', 'featured'][idx % 4],
  tags: ['Honeymoon', 'Family', 'Adventure'].slice(0, 2 + idx % 2),
  ...
  trending: idx % 4 === 0,
  details: {
    line1: `Key detail: Premium quality travel package with verified certifications`,
    line2: `Additional info: Available 24/7 with instant confirmation and support`,
    line3: `Note: Member exclusive pricing with loyalty points on every transaction`,
    stat1: { label: 'Rating', value: `${(4 + Math.random()).toFixed(1)}★` },
    stat2: { label: 'Users', value: `${(100 + idx * 50).toLocaleString()}` },
...
export const TravelDealsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const tabs = [{"label":"Flights","icon":"✈️"},{"label":"Hotels","icon":"🏨"},{"label":"Packages","icon":"🗺️"}];
  const sortOptions = ['default', 'price_low', 'price_high', 'rating', 'popular', 'newest'];

  return (
    <GradientBackground>
      <PremiumHeader title="Travel Deals" subtitle="🔥 Premium Experience" onBack={() => navigation.goBack()} transparent />
                ...
                <>
                  ...
                  <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                    ...
                    >
                      ...
                      <Text style={styles.bannerTitle}>Flash Flight Sale</Text>
                      <Text style={styles.bannerSubtitle}>Up to 50% off on 500+ routes</Text>
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
                    <View style={styles.sectionHeader}>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>✈️ Flight Deals</Text>
                ...
                <>
                  ...
                  <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                    ...
                    >
                      ...
                      <Text style={styles.bannerTitle}>Hotel Mega Sale</Text>
                      <Text style={styles.bannerSubtitle}>Up to ₹3,000 off per night</Text>
                      <View style={styles.bannerStats}>
                        <View style={styles.bannerStat}>
                          <Text style={styles.bannerStatValue}>10+</Text>
                          <Text style={styles.bannerStatLabel}>Options</Text>
                        </View>
                        <View style={styles.bannerStat}>
                          <Text style={styles.bannerStatValue}>4.5★</Text>
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
                    <View style={styles.sectionHeader}>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🏨 Hotel Deals</Text>
              ...
              {selectedTab === 2 && (
                <>
                  ...
                  <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                    ...
                    >
                      <Text style={styles.bannerEmoji}>🗺️</Text>
                      <Text style={styles.bannerTitle}>Holiday Packages</Text>
                      <Text style={styles.bannerSubtitle}>Flights + Hotels + Activities</Text>
                      <View style={styles.bannerStats}>
                        <View style={styles.bannerStat}>
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
                    <View style={styles.sectionHeader}>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🗺️ Package Deals</Text>
