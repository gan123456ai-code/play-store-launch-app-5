...
const bookingsData = Array.from({ length: 15 }, (_, i) => ({
  id: String(i + 1),
  title: `Booking ${i + 1}`,
  subtitle: `Flight/Hotel/Train booking details`,
  description: `Booking ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['✈️', '🏨', '🚂', '🚌'][i % 4],
  price: 2000 + (i * 1500),
  originalPrice: 3000 + (i * 1500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Confirmed', 'Completed', 'Cancelled', 'Pending'],
  features: ['E-Ticket', 'Refundable', 'Insured'],
  ...
  category: ['Flights', 'Hotels', 'Trains', 'Buses'][i % 4],
  amount: 2000 + (i * 1500),
  imageEmoji: ['✈️', '🏨', '🚂', '🚌'][i % 4],
  details: {
...
const upcomingTripsItems = [
  { id: '0', title: 'Delhi → Goa', subtitle: '25 Dec • IndiGo', icon: '✈️', color: '#6366F1', value: '₹4,299' },
  { id: '1', title: 'Taj Mahal Palace', subtitle: '28 Dec • 3 nights', icon: '🏨', color: '#EC4899', value: '₹15,999' },
  { id: '2', title: 'Delhi → Mumbai', subtitle: '5 Jan • Rajdhani', icon: '🚂', color: '#06B6D4', value: '₹1,850' },
];


export const AllBookingsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'flights', 'hotels', 'trains', 'buses'];
  const tabs = [{ id: 'upcomingTrips', label: 'Upcoming Trips' }];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <View>
              <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>📅 Upcoming Trips</Text>
              <Text style={{ fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 }}>Your next adventures</Text>
            </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16 }}>
            {upcomingTripsItems.map((item, idx) => (
              <GlassCard
                key={item.id}
                onPress={() => { haptics.medium(); navigation.navigate('BookingDetails'); }}
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(300)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All My Bookings</Text>
          </View>
          {loading ? (
                ...
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      ...
                      navigation.navigate('BookingDetails', { itemId: item.id, item });
                    }}
