...
const cancelReasonsData = Array.from({ length: 9 }, (_, i) => ({
  id: String(i + 1),
  title: `Reason ${i + 1}`,
  subtitle: `Select cancellation reason`,
  description: `Cancel reason ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['📅', '💰', '🏥', '🌧️', '👨‍👩‍👧'][i % 5],
  price: 0 + (i * 500),
  ...
  category: ['Personal', 'External'][i % 2],
  amount: 0 + (i * 500),
  imageEmoji: ['📅', '💰', '🏥', '🌧️', '👨‍👩‍👧'][i % 5],
  details: {
...
export const CancelBookingScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'personal', 'external'];
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(100)}>
          <SearchBar
            ...
            placeholder="Search cancel booking..."
            showFilter
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All Cancel Booking</Text>
          </View>
          {loading ? (
            <SkeletonList count={5} />
          ) : (
            <View style={{ paddingHorizontal: 16 }}>
              {cancelReasonsData.map((item, index) => (
                <Animated.View key={item.id} entering={FadeInDown.duration(400).delay(index * 50)}>
                  <GlassCard
                    onPress={() => {
                      haptics.medium();
                      addToHistory(item.title);
                      navigation.navigate('RefundStatus', { itemId: item.id, item });
                    }}
