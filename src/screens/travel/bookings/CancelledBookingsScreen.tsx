...
const cancelledData = Array.from({ length: 5 }, (_, i) => ({
  id: String(i + 1),
  title: `Cancelled Booking ${i + 1}`,
  subtitle: `Refund processed`,
  description: `Cancelled booking ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['✈️', '🏨', '🚂'][i % 3],
  price: 2000 + (i * 500),
  originalPrice: 3000 + (i * 500),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Refunded', 'Processing'],
  features: ['Refund Initiated'],
...
export const CancelledBookingsScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const filters = ['all', 'flights', 'hotels'];
