...
const insuranceData = Array.from({ length: 4 }, (_, i) => ({
  id: String(i + 1),
  title: `Insurance Plan ${i + 1}`,
  subtitle: `Comprehensive travel coverage`,
  description: `Insurance plan ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['🛡️', '⭐', '💎', '🏆'][i % 4],
  price: 299 + (i * 300),
  originalPrice: 1299 + (i * 300),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Basic', 'Standard', 'Premium'],
  features: ['Trip Cancellation', 'Medical', 'Baggage'],
  ...
  category: ['Domestic', 'International'][i % 2],
  amount: 299 + (i * 300),
  imageEmoji: ['🛡️', '⭐', '💎', '🏆'][i % 4],
  details: {
...
export const TravelInsuranceScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
