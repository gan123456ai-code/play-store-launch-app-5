...
const historyData = Array.from({ length: 20 }, (_, i) => ({
  id: String(i + 1),
  title: `Past Booking ${i + 1}`,
  subtitle: `Completed trip`,
  description: `Historical booking ${i + 1}. This includes comprehensive details about pricing, availability, features, amenities, and user reviews. Each item has been carefully curated to provide the best experience for our premium users.`,
  icon: ['✈️', '🏨', '🚂', '🚌'][i % 4],
  price: 2000 + (i * 1000),
  originalPrice: 3000 + (i * 1000),
  rating: (3.5 + (i % 15) / 10).toFixed(1),
  reviews: 50 + i * 30,
  tags: ['Completed', 'Cancelled'],
  features: ['Review Added', 'Photos Added'],
...
export const BookingHistoryScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(600).delay(200)}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: theme.colors.text, fontSize: 18 }]}>🔥 All History</Text>
          </View>
          {loading ? (
