...
const section0Items = Array.from({ length: 12 }, (_, idx) => ({
  ...
  trending: idx % 4 === 0,
  details: {
    line1: `Key detail: Premium quality country visa with verified certifications`,
...
const section1Items = Array.from({ length: 5 }, (_, idx) => ({
  id: `item_${idx}`,
  title: `Visa Application ${idx + 1}`,
  subtitle: `Application status`,
  description: `Detailed description for visa application ${idx + 1}. This premium item offers exceptional quality and value. Features include top-rated service, verified reviews from thousands of users, competitive pricing with exclusive member discounts, and 24/7 customer support. Available across multiple locations with easy booking and instant confirmation. Rated ${(4 + Math.random()).toFixed(1)} stars by over ${100 + idx * 50} verified users.`,
  icon: ['📋', '⏳', '✅', '❌'][idx % 4],
  amount: 1000 + idx * 500,
  rating: Number((3.8 + (idx % 12) / 10).toFixed(1)),
  reviews: 50 + idx * 25,
  status: ['active', 'pending', 'completed', 'featured'][idx % 4],
  tags: ['Submitted', 'Processing', 'Approved', 'Rejected'].slice(0, 2 + idx % 2),
  ...
  trending: idx % 4 === 0,
  details: {
    line1: `Key detail: Premium quality visa application with verified certifications`,
    line2: `Additional info: Available 24/7 with instant confirmation and support`,
    line3: `Note: Member exclusive pricing with loyalty points on every transaction`,
    stat1: { label: 'Rating', value: `${(4 + Math.random()).toFixed(1)}★` },
    stat2: { label: 'Users', value: `${(100 + idx * 50).toLocaleString()}` },
...
export const VisaServicesScreen: React.FC<{ navigation: any; route?: any }> = ({ navigation, route }) => {
  ...
  const tabs = [{"label":"Apply","icon":"🇺🇸"},{"label":"Track","icon":"📋"}];
  const sortOptions = ['default', 'price_low', 'price_high', 'rating', 'popular', 'newest'];

  return (
    <GradientBackground>
      <PremiumHeader title="Visa Services" subtitle="🛂 Premium Experience" onBack={() => navigation.goBack()} transparent />
                ...
                <>
                  ...
                  <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                    ...
                    >
                      ...
                      <Text style={styles.bannerTitle}>Visa Services</Text>
                      <Text style={styles.bannerSubtitle}>Easy visa for 50+ countries</Text>
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
                    <View style={styles.sectionHeader}>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>🇺🇸 Visa Applications</Text>
              ...
              {selectedTab === 1 && (
                <>
                  ...
                  <Animated.View entering={FadeInDown.duration(400).delay(100)}>
                    ...
                    >
                      <Text style={styles.bannerEmoji}>📋</Text>
                      <Text style={styles.bannerTitle}>Track Your Visa</Text>
                      <Text style={styles.bannerSubtitle}>Real-time status updates</Text>
                      <View style={styles.bannerStats}>
                        <View style={styles.bannerStat}>
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
                    <View style={styles.sectionHeader}>
                      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>📋 Track Application</Text>
